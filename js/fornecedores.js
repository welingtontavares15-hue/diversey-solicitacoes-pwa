...
const Fornecedores = {
    ...
    getLinkedUserBySupplier(supplier, users = DataManager.getUsers()) {
        ...
        const supplierEmails = (typeof Utils.extractOperationalEmailRecipients === 'function')
            ? Utils.extractOperationalEmailRecipients(supplier.email || '', supplier.emails || [], supplier.notificationEmails || [])
            : [normalizeEmail(supplier.email || '')].filter(Boolean);
        ...
        return (users || []).find((user) => {
            ...
            return supplierEmails.includes(normalizeEmail(user.email));
        }) || null;
    },
        ...
        const content = `
            ...
            <div class="modal-body">
                <form id="supplier-form">
                    ...
                    <div class="form-group">
                        <label for="supplier-email">Email(s) *</label>
                        <input type="text" id="supplier-email" class="form-control"
                               value="${Utils.escapeHtml(supplier?.email || '')}"
                               placeholder="email1@fornecedor.com; email2@fornecedor.com" required>
                    </div>
    ...
    async save() {
        ...
        const supplierRecipients = typeof Utils.extractOperationalEmailRecipients === 'function'
            ? Utils.extractOperationalEmailRecipients(email)
            : [email].filter((value) => Utils.isValidEmail(value));

        if (supplierRecipients.length === 0) {
            Utils.showToast('Informe ao menos um e-mail válido para o fornecedor', 'warning');
            return;
        }
        ...
        const primaryEmail = supplierRecipients[0];
        const normalizedEmail = (typeof DataManager.normalizeEmail === 'function')
            ? DataManager.normalizeEmail(primaryEmail)
    ...
    async resetPassword(id) {
        ...
        const targetEmail = (typeof Utils.getPrimaryOperationalEmail === 'function')
            ? Utils.getPrimaryOperationalEmail(supplier.email || '', authUser.email || '')
            : String(supplier.email || authUser.email || '').trim();
