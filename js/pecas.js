...
const Pecas = {
    ...
    openForm(id = null) {
        ...
        `<option value="${cat}" ${part?.categoria === cat ? 'selected' : ''}>${cat}</option>`
                    ...
                    <div class="form-group">
                        <label for="part-fornecedor">Fornecedor</label>
                        <select id="part-fornecedor" class="form-control">
                            <option value="sup-ebst"   ${(part?.fornecedorId || 'sup-ebst') === 'sup-ebst'   ? 'selected' : ''}>EBST</option>
                            <option value="sup-hobart" ${part?.fornecedorId === 'sup-hobart' ? 'selected' : ''}>Hobart</option>
                        </select>
                    </div>
                </form>
            </div>
    ...
    async savePart() {
        ...
        const fornecedorId = (document.getElementById('part-fornecedor')?.value || 'sup-ebst').trim();
    ...
    createAutocomplete(containerId, options = {}) {
        ...
        const fornecedorId = options.fornecedorId || null;
        ...
        const debouncedSearch = Utils.debounce((query) => {
            ...
            setTimeout(() => {
                const result = DataManager.searchParts(query, 1, 30, fornecedorId);
