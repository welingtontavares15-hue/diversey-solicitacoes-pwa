...
describe('Report filters', () => {
    ...
    it('renders the supplier filter in the modern report layout with aligned grid classes', () => {
        const { relatorios, applyReportsModernization } = createRelatoriosEnvironment();
        applyReportsModernization();

        relatorios.filters = {
            ...relatorios.getDefaultFilters(),
        ...
        expect(document.querySelector('.report-filters-grid-wide')).not.toBeNull();
        expect(document.querySelector('.report-filter-field--supplier')).not.toBeNull();
        expect(document.querySelector('.report-filter-field--actions')).not.toBeNull();
    });
});
